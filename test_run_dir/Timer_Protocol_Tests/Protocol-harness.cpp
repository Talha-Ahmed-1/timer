...
#endif
#include <iostream>
class Protocol_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    Protocol_api_t(VProtocol* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_prescalar)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_compare)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_reset)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_enable)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_intr_cmp)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Protocol.reset"] = 0;
    }
#if VM_TRACE
...
static Protocol_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VProtocol* top = new VProtocol;
    std::string vcdfile = "test_run_dir/Timer_Protocol_Tests/Protocol.vcd";
    ...
    Verilated::mkdir("test_run_dir/Timer_Protocol_Tests/logs");
    VerilatedCov::write("test_run_dir/Timer_Protocol_Tests/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
