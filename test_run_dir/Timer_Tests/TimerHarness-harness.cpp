...
#endif
#include <iostream>
class TimerHarness_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    TimerHarness_api_t(VTimerHarness* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_intr_ovf)));
        ...
        sim_data.signal_map["TimerHarness.reset"] = 0;
    }
#if VM_TRACE
...
static TimerHarness_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VTimerHarness* top = new VTimerHarness;
    std::string vcdfile = "test_run_dir/Timer_Tests/TimerHarness.vcd";
    ...
    Verilated::mkdir("test_run_dir/Timer_Tests/logs");
    VerilatedCov::write("test_run_dir/Timer_Tests/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
